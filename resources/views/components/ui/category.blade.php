...
@php
    ...
    $image = !empty($icon) ? asset('storage/') .'/'. $icon : '/img/other.svg';
@endphp
...
>
    <a href="{{ route('categoryPage', ['catId' => $id]) }}" class="flex items-center gap-2 py-5 px-6">
        <div class="{{ $classIcon }} transition-all duration-300 bg-[#004972] group-hover:bg-[#F2F9FB] group-focus:bg-[#F2F9FB]" style="
            min-width: 40px;
            height: 25px;
            mask: url({{ $image }}) no-repeat center / contain !important;
            -webkit-mask: url({{ $image }}) no-repeat center / contain !important;
        "></div>
        <p>{{ $title }}</p>
    </a>
</div>
